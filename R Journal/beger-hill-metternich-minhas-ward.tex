...
the \CRANpkg{spduration} \citep{beger2016spduration} package for {\sf R} that allows for time-varying covariates. The statistical model accounts for units that are immune to a
...
Additionally, risk factors might change over time which requires estimators 
...
and risk equations, and test statistics with \(p\)-values. The estimates are based on maximum likelihood estimation and standard errors are derived using the Hessian matrix of the optimization results. The \code{spdur} parameter estimates can be
...
\code{predict} calculates the conditional hazard, but the probability that an 
...
for their application. Here we use \code{predict} to estimate conditional cure 
