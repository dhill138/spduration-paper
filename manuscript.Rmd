...
Using the \code{summary} function on either model object will produce standard output showing the model formula, estimates for the duration and risk equations, and test statistics with $p$-values. These can be exported by calling \code{xtable} directly on the "spdur" object to produce Table \ref{loglog-table}.
...
One may also use the \code{AIC} and \code{BIC} function to calculate the information criterion statistics for \code{spdur} objects. 
...
Lastly, here is a comparision to regular logistic regression. 
...
glm2_test_p   <- predict(glm_model2, newdata = coup_test, type = "response")
glm2_test_p   <- glm2_test_p[!is.na(glm2_test_p)]

obs_y <- coup_test[complete.cases(coup_test), "coup"]

library("separationplot")
separationplot(glm2_test_p,   obs_y)

brier(obs_y, glm2_test_p)
auc_roc(obs_y, glm2_test_p)
auc_roc(obs_y, weib2_test_p)
auc_roc(obs_y, loglog2_test_p)
auc_pr(obs_y, glm2_test_p)
auc_pr(obs_y, weib2_test_p)
auc_pr(obs_y, loglog2_test_p)
```
